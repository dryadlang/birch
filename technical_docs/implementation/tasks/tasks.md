...
- **Dependência**: Nenhuma
- **Descrição**:arrays precisam de métodos funcionais.
- **Ação**:
    1. Implementar `push`, `pop`, `map(fn)`, `filter(fn)` em Rust.
    2. Expor como métodos nativos no Runtime para o tipo `Value::Array`.
